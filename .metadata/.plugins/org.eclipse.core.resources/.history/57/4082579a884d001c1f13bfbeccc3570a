...
function confirmCheck(){
	if(confirm("정말 삭제 하시겠습니까?")==true) location.href='/Intranet/delete?bno=${bno}&num=${num}'
}

function fileDownload(file_no){
	
	location.href='/Intranet/fileDownload?file_no=' + file_no;
}
...
function replyRegister() { //form 데이터 전송 --> 반드시 serialize()를 해야 한다.
	...
	$.ajax({
		url : "/Intranet/replyInsert?option=I",
		type : "post",
		dataType : "json",
		data : queryString,
		success : replyList,
...
function pageStart() {
	...
	$.ajax({
		url : "/Intranet/replyInsert?option=L",
		type : "post",
		dataType : "json",
		data : queryString,
		success : replyList,
...
function replyModify(replyBno) {
	...
	$("#btn_replyModify").on("click", function(){
		...
		$.ajax({
			url : "/Intranet/replyInsert?option=U",
			type : "post",
			dataType : "json",
			data : queryString,
			success : replyList,
...
<div class="navi_top">
 ...
 <ul class="main_menu">
            ...
            <li><a href="/Intranet/sourceView">소스 보기</a>
            </li>
</ul>
</div>
...
<div class="navi_bottom">
				<c:if test="${viewPrev != 0 }">
					&nbsp;&nbsp;<a href="/Intranet/view?bno=${viewPrev}&num=${num}&searchType=${searchType}&keyword=${keyword}" onMouseover="this.style.textDecoration='underline';" 
						onmouseout="this.style.textDecoration='none';">이전글 ▼</a> &nbsp;&nbsp;
				</c:if>
					
				<a href="/Intranet/listPage?num=${num}&searchType=${searchType}&keyword=${keyword}" onMouseover="this.style.textDecoration='underline';" 
						onmouseout="this.style.textDecoration='none';">목록보기</a> &nbsp;&nbsp;

				<c:if test = "${viewNext != 0 }">
				<a href="/Intranet/view?bno=${viewNext}&num=${num}&searchType=${searchType}&keyword=${keyword}" onMouseover="this.style.textDecoration='underline';" 
						onmouseout="this.style.textDecoration='none';">다음글 ▲</a> &nbsp;&nbsp;
				</c:if>

				<a href="/Intranet/write?num=${num}" onMouseover="this.style.textDecoration='underline';" 
						onmouseout="this.style.textDecoration='none';">글 작성</a> &nbsp;&nbsp;

				<c:if test="${msg == true}">
				<a href="/Intranet/modify?bno=${view.bno}&num=${num}&searchType=${searchType}&keyword=${keyword}" onMouseover="this.style.textDecoration='underline';" 
