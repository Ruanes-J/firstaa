...
function search(){
	...
	location.href = '/member/listPage?num=${num}&searchType=' + searchType + '&keyword=' + keyword;
...
<c:forEach items="${list}" var="list">
 <tr onMouseover="this.style.background='#46D2D2';" 
  ...
  <td style="text-align:left;"><a id="hypertext" href="/member/view?bno=${list.bno}&num=${num}&searchType=${searchType}&keyword=${keyword}" onMouseover='this.style.textDecoration="underline"'  
...
<div class="navi_bottom">
	[<a id="hypertext" href="/member/write?num=${num}" onMouseover="this.style.background='#96FFFF'; this.style.textDecoration='underline';" 
						onmouseout="this.style.background='white'; this.style.textDecoration='none';">글 작성</a>]
	[현재 ${num}/${page.endPageNum} 페이지] [
	<c:if test="${page.prev}">
 		<span><a href="/member/listPage?num=${page.startPageNum - 1}">◀</a></span>
	</c:if>

	<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}" var="cnt">

  		<c:if test="${cnt != num}">
  			<span><a href="/member/listPage?num=${cnt}">${cnt}</a></span>
  		</c:if>
  		<c:if test="${cnt == num}">
  			<span>${num}</span>
  		</c:if>
	...
	<c:if test="${page.next}">
 		<span><a href="/member/listPage?num=${page.endPageNum + 1}">▶</a></span>
	</c:if>
	]<br>
</div>
